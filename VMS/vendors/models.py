...
class Vendor(models.Model):
    name =  models.CharField(max_length=50, blank=True, null=True) # Vendor's name.
...
class PurchaseOrder(models.Model):
    po_number = models.CharField(unique=True) # Unique number identifying the PO.
    vendor = models.ForeignKey(to=Vendor, on_delete=models.SET_NULL) # Link to the Vendor model.
    ...
    status = models.CharField(choices=CHOICES_STATUS, blank=False) # Current status of the PO (e.g., pending, completed, canceled).
    quality_rating = models.FloatField(blank=True) # Rating given to the vendor for this PO (nullable).
    issue_date = models.DateTimeField(blank=False) # Timestamp when the PO was issued to the vendor.
    acknowledgment_date = models.DateTimeField(blank=True) #, nullable - Timestamp when the vendor acknowledged the PO
#end class
